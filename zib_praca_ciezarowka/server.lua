...
function startJob(model)
	...
	if selectedTruck.trailer and not session.bigSetUpgrade then
       return
    end
...
if session
    and session.accelerationUpgrade then

    local handling =
        getVehicleHandling(veh)
    ...
    if handling then
        ...
        setVehicleHandling(
            veh,
            "engineAcceleration",
            handling.engineAcceleration + 20,
            false
        )
...
for _, entry in ipairs(randomCargo) do
    ...
    -- reszta obecnej logiki

end
				local function getRandomCargoList()

    local available = {}

    for k, cargo in ipairs(Config.Cargo) do
...
function selectCargo(cargoID)
    ...
    if not isElement(data.vehicle) then
        return
    end

    ----------------------------------------------------
    -- POBIERAMY INFORMACJE O POJEŹDZIE
    ----------------------------------------------------
    ...
    for _, truck in ipairs(Config.Trucks) do
        ...
        if tonumber(truck.model)
            == tonumber(vehicleModel) then
    ...
    ----------------------------------------------------
    -- ULEPSZENIE ŁADOWNOŚCI
    ----------------------------------------------------

    local session =
        PlayerSessions[player]

    local capacityUpgrade = false

    if session then
        capacityUpgrade =
            session.capacityUpgrade == true
    end
    ...
    ----------------------------------------------------
    -- SPRAWDZENIE MAKSYMALNEJ WAGI
    --
    -- WAŻNE:
    -- limit 25T / 45T sprawdzamy na WADZE BAZOWEJ.
    --
    -- +5T z ulepszenia nie zwiększa
    -- maksymalnego limitu systemowego.
    ----------------------------------------------------

    if baseWeight >
        truckConfig.maxCapacity then

        outputChatBox(
            "Za ciężko, nie załadujemy pojazdu.",
            player,
            255,
            0,
            0
        ...
        outputChatBox(
            "Waga ładunku: "
            ..baseWeight
            .." kg. Maksymalnie dla tego pojazdu: "
            ..truckConfig.maxCapacity
            .." kg.",
            player,
            255,
            100,
    ...
    ----------------------------------------------------
    -- ŁADUNEK MOŻE ZOSTAĆ ZAŁADOWANY
    ----------------------------------------------------
    ...
    data.cargo = {
        ...
        legalCapacity =
            truckConfig.legalCapacity,

        maxCapacity =
            truckConfig.maxCapacity,

        category =
            truckConfig.category

    }

    ----------------------------------------------------
    -- KOMUNIKAT O ULEPSZENIU
    ----------------------------------------------------

    if capacityUpgrade
        and finalWeight > truckConfig.maxCapacity then

        outputChatBox(
            "Jakoś upchnęliśmy, ale stawiasz frytki.",
            player,
            255,
            150,
            0
