...
local banditoFont = guiCreateFont("Borscha-Regular.ttf", 14)
...
infoText = "------ ZMIENNY23 ------ \n\n* Polski Serwer Freeroam\n* CAR PASS: F4\n* Zmienni & Bezimienni 2019\n\nMożesz zgłosić własne propozycje pojazdów!\n\nGrając jako Guest (Gość) nie skorzystasz ze wszystkich funkcji oferowanych na serwerze.\n\n\n\n\nby Adim129_Fast\n\n\nZMIENNY23 FREEROAM"
...
function loginPanel()
		...
		if xmlFile then
			...
			if (status == "true") then
				...
				mainWindow = guiCreateWindow(screenWidth/2-mainWidth/2,screenHeight/2-mainHeight/2,mainWidth,mainHeight,"ZMIENNY23 FREEROAM | LOGIN PANEL",false)
				...
				guiSetFont(btnToggleRegister, banditoFont)
...
function onClickGuest(button,state)
	if(button == "left" and state == "up") then
		if (source == btnPlayAsGuest) then
			...
			showCursor(false)
			outputChatBox("#0000FF* #FFFFFFGrasz jako #ABCDEFGość#FFFFFF!",255,255,255,true)
			if blackLoginScreen == true then
