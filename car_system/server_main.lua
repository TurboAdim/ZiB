...
function spawnVehicle(id, player, showMessage, warpInto)
	...
	if isResourceRunning("mysql") then
		if checkVeh (player) + 1 > 37 then outputCarSystemError ("Можно спавнить только 37 авто.", player) return end
...
function spawnVehicleCallback(result, args)
	...
	if (args.showMessage) then
		outputCarSystemInfo("Ваш транспорт заспавнен.", args.player)
	end
...
function removeMyVehicle(id)
	...
	if isElement(vehicle) then
		...
		elseif getElementData(vehicle, "hasIllegalItems") then
			outputCarSystemError("Вы не можете убрать данное транспортное средство, так как в нем находятся незаконные предметы", client)
			return
		end
		destroyVehicle(vehicle)
		outputCarSystemInfo("Ваш транспорт убран.", client)
	else
...
function buyNewVehicle(model, r1, g1, b1, r2, g2, b2, shopID, region)
	...
	if (not notSlottingCar[model]) and isResourceRunning("house") then
		...
		if (usedParkingLots > availableParkingLots) then
			outputCarSystemError("Вы не можете купить больше "..(availableParkingLots+1).." машин.", client)
			outputCarSystemError("У вас недостаточно парковочных мест.", client)
			outputCarSystemError("В данный момент у вас "..usedParkingLots.." машин.", client)
			return false
		end
	end
	...
	if (getThisModelCarsCount(accName, model) > 3) then
	...
	if isResourceRunning("bank") then
		...
		outputCarSystemInfo("Вы купили "..getVehicleModName(model).." за "..explodeNumber(cost)..currencyToSymbol(currency), client)
		moneyText = string.format("bank %i%s", exports.bank:getPlayerBankMoney(client, currency), (currency or "PLN"))
	else
		takePlayerMoney(client, cost)
		outputCarSystemInfo("Вы купили "..getVehicleModName(model).." за "..cost.." руб.", client)
...
function BuyVehicleByUsed(player, data, position)
	...
	if isResourceRunning("bank") then
		if (exports.bank:getPlayerBankMoney(player, "PLN") < data.price) then
	...
	if (getThisModelCarsCount(accName, data.model) > 3) then
	...
	local moneyText = ""
	if isResourceRunning("bank") then
		exports.bank:takePlayerBankMoney(player, data.price, "PLN")
		outputCarSystemInfo("Вы купили "..getVehicleModName(data.model).." за "..explodeNumber(data.price).." руб.", player)
		moneyText = "bank "..exports.bank:getPlayerBankMoney(player, "PLN").."PLN"
	else
		takePlayerMoney(player, data.price)
		outputCarSystemInfo("Вы купили "..getVehicleModName(data.model).." за "..data.price.." руб.", player)
...
--if not checkCarInColshape(args.vehicle, result, args.player) then return end
	...
	if (not notSoldableCar[result.model]) and (not overPricedCar[result.model]) and (not isBoat[result.model]) and (not isHeli[result.model]) and isResourceRunning("car_usedauto") then
		if (args.money*1.2 >= 100000) then
			...
			outputCarSystemWarning("Внимание! Б/у автосалон не выставляет на продажу автомобили дешевле 100 000 рублей.", args.player)
		end
	end	
	...
	exports.bank:givePlayerBankMoney(args.player, args.money, "PLN")
	outputCarSystemInfo("Вы продали Ваш транспорт за "..explodeNumber(args.money).." руб. Деньги переведены на ваш банковский счет.", args.player)
	
	outputDebugString(string.format("[CAR-SHOP][SELL] %s (acc %s bank %s) sold %s (%i, %s, %i) for %i",
		getPlayerName(args.player), accName,exports.bank:getPlayerBankMoney(args.player, "PLN"), getVehicleModName(result.model), (result.ID or args.id),result.licensep,result.model, args.money))
...
function resetVehicleHandling(id)
	...
	if destroy then
		outputCarSystemInfo("Вы сбросили хэндлинг на "..getVehicleModName(vehModel)..".", client)
		spawnVehicle(id, client, false, false)
	else
		outputCarSystemInfo("Вы успешно сбросили хэндлинг на этой машине.", client)
	end
end
...
function warpMyVehicle(id)
	...
	if isElement(vehicle) then
		...
		outputCarSystemInfo("Ваш транспорт доставлен к вам.", client)
	else
		outputCarSystemInfo("Ваш транспорт #FF0000не заспавнен", client)
	end
end
...
function outputCarSystemInfo(text, player)
	outputChatBox("#4080bf[Гараж] #f2f2f2"..text:gsub("#цв#", "#f2f2f2"), player, 38, 122, 216, true)
end
function outputCarSystemWarning(text, player)
	outputChatBox("#ffcc00[Гараж] #f2f2f2"..text:gsub("#цв#", "#f2f2f2"), player, 38, 122, 216, true)
end
function outputCarSystemError(text, player)
	outputChatBox("#ff3333[Гараж] #f2f2f2"..text:gsub("#цв#", "#f2f2f2"), player, 38, 122, 216, true)
end
...
function wipeAccount(accName)
	...
	for index, car in ipairs(data) do
		...
		outputDebugString(string.format("[CAR-SYSTEM] Deleted car ID %i, owner %s, licensep %s, model %i, name \"%s\", cost %i %s",
			car.ID, car.owner, car.licensep, car.model, getVehicleModName(car.model), cost, (currency or "PLN")
		))
		data[index].cost = exports.bank:convertCurrency(cost, (currency or "PLN"), "PLN")
